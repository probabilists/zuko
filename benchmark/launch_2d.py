...
def experiment(flow: str, dataset: str, args: argparse.Namespace) -> None:
    build = FLOWS[flow]
    ...
    try:
        model = build(trainset.shape[-1], residual=args.residual, **kwargs)
    except TypeError:
        model = build(trainset.shape[-1], **kwargs)
    ...
    run = wandb.init(
        ...
        config=dict(flow=flow, dataset=dataset, **vars(args)),
    )
...
if __name__ == "__main__":
    ...
    for flow in FLOWS:
        for dataset in DATASETS:
            job = dawgz.job(experiment, cpus=4, ram="16GB", time="06:00:00")(flow, dataset, args)
            jobs.append(job)
